      ...
      {imageLinkList.map((item, index) => (
        <div key={index} className="w-full lg:w-1/2 my-4 flex justify-center">
          {/*<a href={item.link} target="_blank" rel="noopener noreferrer" className="w-full flex justify-center">*/}
            <div className="py-2 px-2 max-w-full w-11/12 mx-2 bg-blue-200 hover:bg-blue-100 transition-colors duration-200 rounded-xl shadow-lg flex items-center space-x-2 relative">
              <img className="block  h-40 w-40 rounded-xl sm:mx-0 sm:shrink-0 left-1" src={item.image} alt="Woman's Face" />
